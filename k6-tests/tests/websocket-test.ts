...
export function setup() {
  ...
  if (users.length > 0) {
    ...
    console.log(
      `Created ${rooms.length} test rooms for WebSocket test (${standardRooms.length} standard, ${privateRooms.length} private, ${directRooms.length} direct)`,
    );
  }
...
export default function (data: { users: UserCredentials[]; rooms: Room[] }) {
  ...
  if (
    ...
    console.log(
      `Starting Socket.io chat session with new user in room ${room.id}`,
    );
    runChatSession(
      WS_BASE_URL,
      authData.token,
    ...
    console.log(
      `Using predefined data: User ${user.nickname} in room ${room.name}`,
    );
    ...
    if (Math.random() > 0.7) {
      ...
      if (userToKick && userToKick.userId) {
        console.log(
          `Attempting to kick user ${userToKick.nickname} from room ${room.name}`,
        );
