 ...
 */
export function loginUser(apiBaseUrl: string, email: string, password: string): { token: string, userId: string } | null {
  const payload = JSON.stringify({
    email,
    password,
  });
 ...
 */
export function getUserProfile(apiBaseUrl: string, token: string): UserProfile | null {
...
export function createRoom(
  ...
  const roomName = name || `Room-${Math.random().toString(36).substring(2, 10)}`;
 ...
 */
export function getRoom(apiBaseUrl: string, token: string, roomId: string | number): Room | null {
  try {
    console.log(`Getting room ${roomId} details from URL: ${apiBaseUrl}/rooms/${roomId}`);
 ...
 */
export function getPublicRooms(apiBaseUrl: string, token: string): {items: Room[], meta: any} | null {
 ...
 */
export function joinRoom(apiBaseUrl: string, token: string, roomId: string | number): boolean {
  try {
    console.log(`Joining room ${roomId} at URL: ${apiBaseUrl}/rooms/${roomId}/join`);
 ...
 * @returns Success status
 */
export function addUserToRoom(apiBaseUrl: string, token: string, roomId: string | number, userId: string | number): boolean {
  try {
    console.log(`Adding user ${userId} to room ${roomId} at URL: ${apiBaseUrl}/rooms/${roomId}/users`);
 ...
 * @returns Success status
 */
export function markRoomAsSeen(apiBaseUrl: string, token: string, roomId: string | number): boolean {
  try {
    console.log(`Marking room ${roomId} as seen with URL: ${apiBaseUrl}/rooms/${roomId}/seen`);
    ...
    if (!success) {
      ...
      console.log(`Successfully marked room ${roomId} as seen with status ${res.status}`);
    }
 ...
 * @returns Success status
 */
export function leaveRoom(apiBaseUrl: string, token: string, roomId: string | number): boolean {
 ...
 */
export function getRoomUsers(apiBaseUrl: string, token: string, roomId: string | number): UserProfile[] | null {
 ...
 * @returns Success status
 */
export function kickUser(apiBaseUrl: string, token: string, roomId: string | number, userId: string | number): boolean {
 ...
 * @returns Success status
 */
export function deleteRoom(apiBaseUrl: string, token: string, roomId: string | number): boolean {
