      ...
      for (const version of socketIoVersions) {
        for (const path of pathFormats) {
          for (const authMethod of authMethods) {
            try {
              console.log(
                `Trying Socket.IO connection with path: ${
                  path || "/"
                }, transport: ${transport}, version: ${version}, auth: ${
                  authMethod.name
                }`,
              );
              ...
              console.log(
                `Handshake response status: ${handshakeResponse.status}`,
              );
              ...
              if (
                ...
                console.log(
                  `Handshake successful with path: ${
                    path || "/"
                  }, transport: ${transport}, auth: ${authMethod.name}`,
                );

                // Extract the Socket.IO session ID from the response
                const handshakeData = extractSocketIOJson(
                ...
                if (handshakeData && handshakeData.sid) {
                  ...
                  const pollingUrlWithAuth =
                    authMethod.urlFormatter(pollingUrl);

                  // Send a join room event
                  console.log(
                    `Sending join_room event for room ${roomIdStr}...`,
                  );
                  ...
                  console.log(
                    `Join room response status: ${joinResponse.status}`,
                  );
                  ...
                  if (joinResponse.status === 200) {
                    ...
                    for (let i = 0; i < messagesToSend; i++) {
                      ...
                      console.log(
                        `Sending message ${
                          i + 1
                        }/${messagesToSend}: ${messageText.substring(
                          0,
                          30,
                        )}...`,
                      );
                      ...
                      console.log(
                        `Message ${i + 1} send response status: ${
                      ...
                      if (i === 0) {
                        check(messageResponse, {
                          "Socket.IO message send successful": (r) =>
                            r.status === 200,
                        });
                      }
                      ...
                      if (i < messagesToSend - 1) {
                        ...
                        console.log(
                          `Waiting ${waitTime}ms before sending next message...`,
                        );
                    ...
                    console.log(
                      `Successfully sent ${messagesToSend} messages via Socket.IO ${transport} at path ${
                        path || "/"
                      }`,
                    );
                    ...
                    console.error(
                      `Failed to join room with status ${joinResponse.status}`,
                    );
                  }
                } else {
                  console.error(
                ...
                console.error(
                  `Handshake failed with status ${handshakeResponse.status}`,
                );
              }
            } catch (error) {
              console.error(
                `Error during Socket.IO ${transport} connection: ${error}`,
              );
            }
          }
        }
