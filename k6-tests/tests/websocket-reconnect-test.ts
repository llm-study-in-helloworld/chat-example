...
export function setup() {
  ...
  console.log(
    `Successfully created ${rooms.length} rooms for reconnection testing`,
  );
...
export default function (data: { users: UserCredentials[]; rooms: Room[] }) {
  ...
  for (let cycle = 1; cycle <= reconnectionCycles; cycle++) {
    console.log(
      `VU ${__VU}: Starting connection cycle ${cycle}/${reconnectionCycles}`,
    );
    ...
    check(handshakeResponse, {
      [`Socket.IO handshake successful (cycle ${cycle})`]: (r) =>
        r.status === 200,
    });
    ...
    if (
      ...
      console.error(
        `Handshake failed with status ${handshakeResponse.status} in cycle ${cycle}`,
      );
    ...
    check(joinResponse, {
      [`Socket.IO join room successful (cycle ${cycle})`]: (r) =>
        r.status === 200,
    });

    if (joinResponse.status !== 200) {
      console.error(
        `Failed to join room with status ${joinResponse.status} in cycle ${cycle}`,
      );
      sleep(5);
      continue;
    }
    ...
    for (let i = 0; i < activePhaseMessages; i++) {
      ...
      const messageText = `Reconnect test message from VU ${__VU}, cycle ${cycle}, msg ${
        i + 1
      }/${activePhaseMessages}`;
      ...
      check(messageResponse, {
        [`Socket.IO message ${i + 1} sent successfully (cycle ${cycle})`]: (
          r,
        ) => r.status === 200,
      });
    ...
    for (let i = 0; i < pollCount; i++) {
      ...
      check(pollResponse, {
        [`Socket.IO polling successful ${i + 1}/${pollCount} (cycle ${cycle})`]:
          (r) => r.status === 200,
      });
  ...
  console.log(
    `VU ${__VU}: Completed all ${reconnectionCycles} reconnection cycles`,
  );
}
