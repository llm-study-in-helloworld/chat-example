...
export function setup(): TestData {
  ...
  const users = Array(5).fill(0).map((_, i) => {
  ...
  if (publicRoom) {
    ...
    console.log(`Created public room: ${publicRoom.name} (ID: ${publicRoom.id})`);
    rooms.push(publicRoom);
    
    // Add some users to the public room
    for (let i = 1; i < users.length; i++) {
      const joinSuccess = joinRoom(API_BASE_URL, users[i].token!, publicRoom.id);
      console.log(`User ${users[i].nickname} ${joinSuccess ? 'joined' : 'failed to join'} public room`);
  ...
  if (privateRoom) {
    ...
    console.log(`Created private room: ${privateRoom.name} (ID: ${privateRoom.id})`);
    ...
    for (let i = 1; i <= maxUsersToAdd; i++) {
      if (users[i] && users[i].userId && users[i].token) {
        const addSuccess = addUserToRoom(API_BASE_URL, adminUser.token!, privateRoom.id, users[i].userId!);
        console.log(`User ${users[i].nickname} ${addSuccess ? 'added to' : 'failed to be added to'} private room`);
  ...
  if (users.length > 1) {
    ...
    if (directRoom) {
      ...
      console.log(`Created direct room: ${directRoom.name} (ID: ${directRoom.id})`);
      rooms.push(directRoom);
      
      // Add second user to the direct room
      if (users[1] && users[1].userId) {
        const addSuccess = addUserToRoom(API_BASE_URL, adminUser.token!, directRoom.id, users[1].userId!);
        console.log(`User ${users[1].nickname} ${addSuccess ? 'added to' : 'failed to be added to'} direct room`);
      }
    }
  }
  ...
  console.log(`Setup completed successfully with ${users.length} users and ${rooms.length} rooms`);
...
export default function(data: TestData) {  
  ...
  try {
    ...
    console.log(`Test run with admin: ${adminUser.nickname} (${adminUser.userId}) and random user: ${randomUser.nickname} (${randomUser.userId})`);
    
    // Select a random room
    const randomRoomIndex = Math.floor(Math.random() * data.rooms.length);
    const randomRoom = data.rooms[randomRoomIndex];
    console.log(`Selected random room: ${randomRoom?.name || 'none'} (${randomRoom?.id || 'none'})`);
    ...
    if (randomUser && randomUser.token) {
      ...
      check(profile, {
        'user profile retrieved': (p) => p !== null && p.id.toString() === randomUser.userId
      });
 ...
 */
function testCreateRoomAndWebSocket(apiBaseUrl: string, wsBaseUrl: string, user: UserCredentials) {
  ...
  if (newRoom) {
    ...
    console.log(`Trying WebSocket connection to room ${newRoom.name} (${newRoom.id})`);
    ...
    for (let i = 0; i < alternativeWsUrls.length; i++) {
      try {
        console.log(`Trying WebSocket URL ${i+1}/${alternativeWsUrls.length}: ${alternativeWsUrls[i]}`);
        runChatSession(
          alternativeWsUrls[i],
          user.token,
          newRoom.id,
        ...
        console.error(`Error in WebSocket chat session with URL ${alternativeWsUrls[i]}: ${error}`);
      }
    }
...
function testMarkRoomAsSeen(apiBaseUrl: string, user: UserCredentials) {
  ...
  if (newRoom) {
    console.log(`Created room for mark seen test: ${newRoom.name} (${newRoom.id})`);
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      console.log(`Attempt ${attempt}/3: Marking room ${newRoom.name} (${newRoom.id}) as seen`);
      try {
        ...
        console.error(`Error marking room as seen (attempt ${attempt}): ${error}`);
      }
 ...
 */
function testJoinRoom(apiBaseUrl: string, user: UserCredentials, adminUser: UserCredentials) {
  ...
  if (testRoom) {
    console.log(`Admin created room for join test: ${testRoom.name} (${testRoom.id})`);
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      console.log(`Attempt ${attempt}/3: User ${user.nickname} joining room ${testRoom.name} (${testRoom.id})`);
      try {
 ...
 */
function testAddUserToRoom(apiBaseUrl: string, adminUser: UserCredentials, userToAdd: UserCredentials) {
  ...
  if (testRoom) {
    console.log(`Admin created room for add user test: ${testRoom.name} (${testRoom.id})`);
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      console.log(`Attempt ${attempt}/3: Adding user ${userToAdd.nickname} (${userToAdd.userId}) to room ${testRoom.name} (${testRoom.id})`);
      try {
        const addSuccess = addUserToRoom(apiBaseUrl, adminUser.token, testRoom.id, userToAdd.userId);
        ...
        console.error(`Error adding user to room (attempt ${attempt}): ${error}`);
      }
      sleep(1); // Wait before trying again
    }
 ...
 */
function testWebSocketConnections(wsBaseUrl: string, user: UserCredentials, apiBaseUrl: string) {
  if (!user.token) return;
  ...
  console.log(`Created room for WebSocket test: ${testRoom.name} (${testRoom.id})`);
  sleep(1);
  ...
  try {
    ...
    if (handshakeResponse.status === 200 && typeof handshakeResponse.body === 'string') {
      console.log(`Handshake successful, response: ${handshakeResponse.body.substring(0, 100)}...`);
      ...
      if (handshakeData && handshakeData.sid) {
        ...
        console.log(`Sending join_room event for room ${testRoom.id} via polling...`);
        ...
        // Send a test message
        console.log(`Sending test message to room ${testRoom.id} via polling...`);
        ...
        // Check for success
        check(null, {
          'Socket.IO join room successful (polling)': () => joinResponse.status === 200,
          'Socket.IO message send successful (polling)': () => messageResponse.status === 200
        });
