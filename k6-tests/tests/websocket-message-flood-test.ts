...
export function setup() {
  ...
  console.log(`Successfully created flood test room: ${room.name} (${room.id})`);
...
export default function(data: { users: UserCredentials[], room: Room | null }) {
  ...
  for (let batch = 0; batch < messageCount / batchSize; batch++) {
    ...
    check(null, {
      ...
      'Socket.IO message batch mostly successful': () => successCount >= batchSize * 0.7
    });
