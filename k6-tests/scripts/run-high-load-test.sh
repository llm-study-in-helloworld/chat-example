...
$DOCKER_COMPOSE_CMD -f docker/docker-compose.yml --profile high-load up -d influxdb grafana --remove-orphans
...
$DOCKER_COMPOSE_CMD -f docker/docker-compose.yml --profile high-load run \
...
if [ "$CLEANUP_K6_ONLY" = true ]; then
  echo "k6 컨테이너만 정리합니다. Grafana와 InfluxDB는 유지됩니다."
else
