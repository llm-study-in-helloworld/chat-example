...
export class AuthService {
  ...
  constructor(
    ...
    this.logger.debug(`Access token expires in: ${this.accessTokenExpiresIn} seconds`, 'AuthService');
  }
  ...
  async validateUser(email: string, password: string): Promise<any> {
      ...
      if (!isPasswordValid) {
        this.logger.warn(`Authentication failed: Invalid password for user ${email}`, 'AuthService');
        throw new UnauthorizedException('Invalid credentials');
      }
      
      this.logger.debug(`User ${email} successfully authenticated`, 'AuthService');
  ...
  async login(user: any, req?: Request) {
    this.logger.debug(`User ${user.email} (ID: ${user.id}) initiating login`, 'AuthService');
      ...
      this.logger.debug(`Previous refresh tokens revoked for user ${user.id}`, 'AuthService');
      
      // Create refresh token
      const userAgent = req?.headers['user-agent'] || 'unknown';
      const ipAddress = req?.ip || req?.socket.remoteAddress || 'unknown';
      this.logger.debug(`Creating refresh token for user ${user.id} from ${ipAddress} with ${userAgent}`, 'AuthService');
      
      const refreshToken = await this.refreshTokenService.createRefreshToken(user, req);
      this.logger.log(`User ${user.email} (ID: ${user.id}) successfully logged in`, 'AuthService');
  ...
  async refreshTokens(userId: number, refreshToken: string, req?: Request) {
      ...
      const userAgent = req?.headers['user-agent'] || 'unknown';
      const ipAddress = req?.ip || req?.socket.remoteAddress || 'unknown';
      this.logger.debug(`Rotating refresh token for user ${userId} from ${ipAddress} with ${userAgent}`, 'AuthService');
      
      const newRefreshToken = await this.refreshTokenService.rotateRefreshToken(refreshToken, req);
      this.logger.debug(`New refresh token created for user ${userId}`, 'AuthService');
      
      // Generate a new access token
      const user = await this.usersService.findById(userId);
      
      if (!user) {
        this.logger.warn(`Token refresh failed: User ${userId} not found`, 'AuthService');
        throw new UnauthorizedException('User not found');
      }
      ...
      this.logger.debug(`New access token created for user ${userId}`, 'AuthService');
      
      // Update the latest token
      this.tokenBlacklistService.setLatestUserToken(user.id, accessToken);
      this.logger.log(`Tokens refreshed successfully for user ${user.email} (ID: ${userId})`, 'AuthService');
  ...
  async validateToken(token: string): Promise<User | null> {
      ...
      try {
        ...
        this.logger.debug(`Token verified for user ID: ${payload.sub}`, 'AuthService');
        
        // Clone or recreate entity manager options to ensure proper context
        const user = await this.usersService.findById(payload.sub);
        
        if (!user) {
          this.logger.warn(`Token validation failed: User with ID ${payload.sub} not found`, 'AuthService');
          return null;
        }
        
        this.logger.debug(`Token successfully validated for user ${user.id}`, 'AuthService');
        return user;
