...
export class TokenBlacklistService {
  ...
  constructor(
    ...
    this.logger.debug(
      "Token blacklist service initialized with hourly cleanup",
  ...
  async blacklistToken(token: string): Promise<void> {
    ...
    try {
      ...
      if (decoded.sub) {
        ...
        this.logger.debug(
          `Token for user ${userId} added to blacklist, expires at ${expiry.toISOString()}`,
          "TokenBlacklistService",
        );
      } else {
        this.logger.debug(
          `Anonymous token added to blacklist, expires at ${expiry.toISOString()}`,
          "TokenBlacklistService",
        );
      }
    } catch (error: unknown) {
      ...
      this.logger.error(
        `Failed to blacklist token: ${errorMessage}`,
        errorStack,
        "TokenBlacklistService",
      );
    } finally {
  ...
  blacklistUserTokens(userId: number): void {
    ...
    try {
      this.logger.debug(
        `Blacklisting all tokens for user ${userId}`,
        "TokenBlacklistService",
      );
      ...
      if (!this.userTokens.has(userId)) {
        this.userTokens.set(userId, new Set());
        this.logger.debug(
          `No existing tokens found for user ${userId}`,
          "TokenBlacklistService",
        );
        return;
      }
      ...
      if (!userTokens || userTokens.size === 0) {
        this.logger.debug(
          `No tokens to blacklist for user ${userId}`,
          "TokenBlacklistService",
        );
        return;
      }
      ...
      for (const token of userTokens) {
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : "Unknown error";
          this.logger.warn(
            `Failed to blacklist a token for user ${userId}: ${errorMessage}`,
            "TokenBlacklistService",
          );
        }
      }
      ...
      this.logger.debug(
        `Successfully blacklisted ${blacklistedCount} tokens for user ${userId}`,
        "TokenBlacklistService",
      );
      ...
      if (this.latestUserTokens.has(userId)) {
        ...
        this.logger.debug(
          `Removed latest token reference for user ${userId}`,
          "TokenBlacklistService",
        );
      }
    } catch (error: unknown) {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error blacklisting tokens for user ${userId}: ${errorMessage}`,
        errorStack,
        "TokenBlacklistService",
      );
    } finally {
  ...
  setLatestUserToken(userId: number, token: string): void {
    ...
    try {
      this.logger.debug(
        `Setting latest token for user ${userId}`,
        "TokenBlacklistService",
      );
      ...
      if (decoded && typeof decoded === "object" && decoded.exp) {
        const expiry = new Date(decoded.exp * 1000);
        this.logger.debug(
          `Latest token for user ${userId} set, expires at ${expiry.toISOString()}`,
          "TokenBlacklistService",
        );
      } else {
        this.logger.debug(
          `Latest token for user ${userId} set with unknown expiry`,
          "TokenBlacklistService",
        );
      }
    } catch (error: unknown) {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error setting latest token for user ${userId}: ${errorMessage}`,
        errorStack,
        "TokenBlacklistService",
      );
    } finally {
  ...
  isBlacklisted(token: string): boolean {
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error checking token blacklist: ${errorMessage}`,
        errorStack,
        "TokenBlacklistService",
      );
      return false;
  ...
  private cleanupExpiredTokens(): void {
    ...
    try {
          ...
          try {
            ...
            if (decoded && typeof decoded === "object" && decoded.sub) {
              ...
              if (this.latestUserTokens.get(userId) === token) {
                this.latestUserTokens.delete(userId);
                this.logger.debug(
                  `Removed expired latest token for user ${userId}`,
                  "TokenBlacklistService",
                );
              }
            }
            ...
            const errorMessage =
              error instanceof Error ? error.message : "Unknown error";
            this.logger.warn(
              `Error during token cleanup: ${errorMessage}`,
              "TokenBlacklistService",
            );
          }
      ...
      this.logger.debug(
        `Cleanup completed: removed ${removedCount} expired tokens`,
        "TokenBlacklistService",
      );
    } catch (error: unknown) {
      const errorMessage =
