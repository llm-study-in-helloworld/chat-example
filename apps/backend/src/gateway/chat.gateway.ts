...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      this.logger.debug(
        `Validating token (${token.substring(0, 10)}...)`,
        "ChatGateway",
      );
      ...
      this.logger.debug(
        `Emitted presence update for user ${user.id}: online`,
        "ChatGateway",
      );
    } catch (e) {
  ...
  async handleDisconnect(client: Socket) {
    ...
    try {
      if (client.data.user) {
        ...
        this.server.emit("user_presence", presenceEvent);
        this.logger.debug(
          `User ${client.data.user.id} disconnected, updated presence to offline`,
          "ChatGateway",
        );
      } else {
  ...
  async handleJoinRoom(
  ...
  ): Promise<SocketSuccessDto | SocketErrorDto> {
    const startTime = Date.now();
    this.logger.logMethodEntry("handleJoinRoom", "ChatGateway");
    this.logger.debug(
      `User ${client.data.user.id} attempting to join room ${payload.roomId}`,
      "ChatGateway",
    );
    ...
    try {
      ...
      if (!canJoin) {
        this.logger.warn(
          `Access denied: User ${client.data.user.id} not allowed to join room ${payload.roomId}`,
          "ChatGateway",
        );
      ...
      this.logger.debug(
        `User ${client.data.user.id} joined room ${payload.roomId}`,
        "ChatGateway",
      );

      // Update last seen timestamp
      await this.roomsService.updateLastSeen(
        client.data.user.id,
        payload.roomId,
      );
      this.logger.debug(
        `Updated last seen timestamp for user ${client.data.user.id} in room ${payload.roomId}`,
        "ChatGateway",
      );
  ...
  async handleNewMessage(client: Socket, payload: CreateMessageDto) {
    ...
    this.logger.debug(
      `User ${client.data.user.id} sending message to room ${payload.roomId}`,
      "ChatGateway",
    );
    ...
    try {
      ...
      if (!canJoin) {
        this.logger.warn(
          `Access denied: User ${client.data.user.id} not allowed to send message to room ${payload.roomId}`,
          "ChatGateway",
        );
        return { error: "접근 권한이 없습니다" };
      }
      ...
      this.logger.debug(
        `Message ${message.id} created in room ${payload.roomId}`,
        "ChatGateway",
      );
      ...
      this.logger.debug(
        `Message ${message.id} broadcasted to room ${payload.roomId}`,
        "ChatGateway",
      );
      ...
      if (message.mentions && message.mentions.length > 0) {
        ...
        const mentionedUserIds = message.mentions.map(
          (mention) => mention.mentionedUser.id,
        );
        this.logger.debug(
          `Message ${message.id} mentions users: ${mentionedUserIds.join(
            ", ",
          )}`,
          "ChatGateway",
        );
        ...
        for (const userId of mentionedUserIds) {
          ...
          this.logger.debug(
            `Mention alert sent to user ${userId}`,
            "ChatGateway",
          );
        }
      }
      ...
      this.logger.error(
        `Error in handleNewMessage: ${error.message}`,
        error.stack,
        "ChatGateway",
      );
  ...
  async handleReplyMessage(
    ...
    this.logger.debug(
      `User ${client.data.user.id} replying to message ${payload.parentId} in room ${payload.roomId}`,
      "ChatGateway",
    );
    ...
    try {
      ...
      if (!canJoin) {
        this.logger.warn(
          `Access denied: User ${client.data.user.id} not allowed to send reply to room ${payload.roomId}`,
          "ChatGateway",
        );
        return { error: "접근 권한이 없습니다" };
      }
      ...
      const parentMessage = await this.messagesService.getMessage(
        payload.parentId,
      );
      if (!parentMessage) {
        this.logger.warn(
          `Reply failed: Parent message ${payload.parentId} not found`,
          "ChatGateway",
        );
      ...
      if (parentMessage.roomId !== payload.roomId) {
        this.logger.warn(
          `Reply failed: Parent message ${payload.parentId} not in room ${payload.roomId}`,
          "ChatGateway",
        );
      ...
      this.logger.debug(
        `Reply message ${message.id} created in room ${payload.roomId}`,
        "ChatGateway",
      );
      ...
      this.server.to(`room:${payload.roomId}`).emit("new_message", message);
      this.logger.debug(
        `Reply message ${message.id} broadcasted to room ${payload.roomId}`,
        "ChatGateway",
      );
      ...
      if (parentMessage.sender.id !== client.data.user.id) {
        ...
        this.logger.debug(
          `Reply alert sent to message author ${parentMessage.sender.id}`,
          "ChatGateway",
        );
      }
      ...
      if (message.mentions && message.mentions.length > 0) {
        ...
        this.logger.debug(
          `Reply message ${message.id} mentions users: ${mentionedUserIds.join(
            ", ",
          )}`,
          "ChatGateway",
        );
      ...
      this.logger.error(
        `Error in handleReplyMessage: ${error.message}`,
        error.stack,
        "ChatGateway",
      );
      const errorResponse: SocketErrorDto = {
  ...
  async handleEditMessage(
    ...
    this.logger.debug(
      `User ${client.data.user.id} editing message ${payload.messageId}`,
      "ChatGateway",
    );
    ...
    try {
      ...
      if (!canEdit) {
        this.logger.warn(
          `Edit denied: User ${client.data.user.id} not allowed to edit message ${payload.messageId}`,
          "ChatGateway",
        );
        const errorResponse: SocketErrorDto = {
      ...
      this.logger.debug(
        `Message ${payload.messageId} updated successfully`,
        "ChatGateway",
      );
      ...
      if (message.roomId) {
        ...
        this.logger.debug(
          `Update for message ${payload.messageId} broadcasted to room ${message.roomId}`,
          "ChatGateway",
        );
      }
      ...
      this.logger.error(
        `Error in handleEditMessage: ${error.message}`,
        error.stack,
        "ChatGateway",
      );
      const errorResponse: SocketErrorDto = {
  ...
  async handleReaction(
    client: Socket,
    payload: ReactionDto,
  ): Promise<ReactionResponseDto | SocketErrorDto> {
    const startTime = Date.now();
    this.logger.logMethodEntry("handleReaction", "ChatGateway");
    this.logger.debug(
      `User ${client.data.user.id} reacting to message ${payload.messageId} with emoji ${payload.emoji}`,
      "ChatGateway",
    );
    ...
    try {
      ...
      this.logger.debug(
        `Reaction ${reaction ? "added" : "removed"} for message ${
          payload.messageId
        }`,
        "ChatGateway",
      );
      ...
      if (message && message.roomId) {
        ...
        this.server
          .to(`room:${message.roomId}`)
          .emit("reaction_updated", updateEvent);
        this.logger.debug(
          `Reaction update for message ${payload.messageId} broadcasted to room ${message.roomId}`,
          "ChatGateway",
        );
      }
      ...
      this.logger.error(
        `Error in handleReaction: ${error.message}`,
        error.stack,
        "ChatGateway",
      );
      const errorResponse: SocketErrorDto = {
