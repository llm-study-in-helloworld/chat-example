...
export class UsersService {
  ...
  async createUser(createUserDto: CreateUserDto): Promise<UserResponseDto> {
    ...
    try {
      this.logger.debug(
        `Creating user with email: ${createUserDto.email}, nickname: ${createUserDto.nickname}`,
        "UsersService",
      );
  ...
  async findByEmail(email: string): Promise<User | null> {
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error finding user by email '${email}': ${errorMessage}`,
        errorStack,
        "UsersService",
      );
      return null;
  ...
  async findById(id: number): Promise<User | null> {
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error finding user by ID ${id}: ${errorMessage}`,
        errorStack,
        "UsersService",
      );
      return null;
  ...
  async updateUser(
    ...
    try {
      this.logger.debug(
        `Updating user profile for user ID: ${user.id}`,
        "UsersService",
      );
      ...
      if (!isPasswordValid) {
        this.logger.warn(
          `Profile update failed for user ${user.id}: Invalid password`,
          "UsersService",
        );
      ...
      this.logger.log(
        `User ${user.id} profile updated successfully`,
        "UsersService",
      );
      return UserResponseDto.fromEntity(user);
    } catch (error: unknown) {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error updating user ${user.id}: ${errorMessage}`,
        errorStack,
        "UsersService",
      );
      throw error;
  ...
  async changePassword(
    ...
    try {
      this.logger.debug(
        `Changing password for user ID: ${user.id}`,
        "UsersService",
      );
      ...
      if (!isPasswordValid) {
        this.logger.warn(
          `Password change failed for user ${user.id}: Invalid current password`,
          "UsersService",
        );
        return false;
      }
      ...
      this.userRepository.merge(user).passwordHash = await this.hashPassword(
      ...
      this.logger.logDatabase(
        "update",
        "User",
        { id: user.id, passwordChanged: true },
        "UsersService",
      );

      this.logger.log(
        `Password changed successfully for user ${user.id}`,
        "UsersService",
      );
      return true;
    } catch (error: unknown) {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error changing password for user ${user.id}: ${errorMessage}`,
        errorStack,
        "UsersService",
      );
      throw error;
  ...
  async deleteUser(user: User, password: string): Promise<boolean> {
    ...
    try {
      this.logger.debug(
        `Deleting user account for user ID: ${user.id}`,
        "UsersService",
      );
      ...
      if (!isPasswordValid) {
        this.logger.warn(
          `User deletion failed for user ${user.id}: Invalid password`,
          "UsersService",
        );
        throw new UnauthorizedException("Invalid password");
      }
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `Error deleting user ${user.id}: ${errorMessage}`,
        errorStack,
        "UsersService",
      );
      throw error;
