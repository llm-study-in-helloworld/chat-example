...
export class RoomsService {
  ...
  async isOwner({
    ...
    this.logger.debug(
      `Checking if user ${userId} is owner of room ${roomId}`,
      "RoomsService",
    );
    ...
    this.logger.debug(
      `User ${userId} is ${isOwner ? "" : "not "}owner of room ${roomId}`,
      "RoomsService",
    );
  ...
  async getUserRooms(userId: number): Promise<RoomResponseDto[]> {
    ...
    this.logger.debug(
      `Found ${roomUsers.length} rooms for user ${userId}`,
      "RoomsService",
    );

    const rooms = await this.formatRoomResponse(
      roomUsers.map((ru) => ru.room),
      userId,
    );
    return rooms;
  }
  ...
  async createRoom({
    ...
    this.logger.debug(
      `Creating ${isDirect ? "direct" : "group"} room with ${
        userIdsSet.size
      } users`,
      "RoomsService",
    );
    ...
    if (isDirect) {
      this.logger.debug(
        `Checking for existing direct room with users: ${Array.from(
          userIdsSet,
        ).join(", ")}`,
        "RoomsService",
      );
    ...
    const result = await this.em.transactional(async (em) => {
      if (isDirect) {
        ...
        if (existingRoom.length > 0) {
          this.logger.debug(
            `Found existing direct room with ID ${existingRoom[0].id}`,
            "RoomsService",
          );
    ...
    const formattedRoom = (
      await this.formatRoomResponse([result], result.ownerId)
    )[0];
    this.logger.log(
      `Room created: ID ${result.id}, name: "${result.name}", owner: ${result.ownerId}`,
      "RoomsService",
    );
  ...
  async getRoomById({
    ...
    this.logger.debug(
      `Fetching room ${roomId} for user ${userId}`,
      "RoomsService",
    );
    ...
    if (!room) {
      this.logger.debug(
        `Room ${roomId} not found or not active`,
        "RoomsService",
      );
      return null;
    }
  ...
  async getRoomUsers(roomId: number): Promise<RoomUserResponseDto[]> {
    ...
    this.logger.debug(
      `Found ${roomUsers.length} users in room ${roomId}`,
      "RoomsService",
    );

    return roomUsers.map((roomUser: RoomUser) =>
      RoomUserResponseDto.fromEntity(roomUser),
    );
  }
  ...
  async addUserToRoom(roomId: number, userId: number): Promise<RoomUser> {
    this.logger.debug(
      `Adding user ${userId} to room ${roomId}`,
      "RoomsService",
    );
    ...
    this.logger.debug(
      `Creating new room user for user ${userId} in room ${roomId}`,
      "RoomsService",
    );
  ...
  async removeUserFromRoom(roomId: number, userId: number): Promise<boolean> {
    ...
    const result = await this.roomUserRepository.nativeDelete({
      room: { id: roomId },
      user: { id: userId },
    });
    this.logger.logDatabase(
    ...
    if (success) {
      ...
      this.logger.debug(
        `User ${userId} was not in room ${roomId}`,
        "RoomsService",
      );
    }
  ...
  async canUserJoinRoom({
    ...
    this.logger.debug(
      `Checking if user ${userId} can join room ${roomId}`,
      "RoomsService",
    );

    const room = await this.roomRepository.findOne(
      { id: roomId },
      { fields: ["ownerId", "isPrivate", "isDirect", "isActive"] },
    );
    if (!room) {
    ...
    if (!isUserInRoom) {
      ...
      this.logger.debug(
        `User ${userId} is not in room ${roomId}. Can join: ${canJoin}`,
        "RoomsService",
      );
      return canJoin;
    }
  ...
  async isUserInRoom({
    ...
    this.logger.debug(
      `Checking if user ${userId} is in room ${roomId}`,
      "RoomsService",
    );
    ...
    this.logger.debug(
      `User ${userId} is ${isInRoom ? "" : "not "}in room ${roomId}`,
      "RoomsService",
    );
  ...
  async updateLastSeen(userId: number, roomId: number): Promise<void> {
    this.logger.debug(
      `Updating last seen time for user ${userId} in room ${roomId}`,
      "RoomsService",
    );
    ...
    if (roomUser) {
      ...
      this.logger.logDatabase(
        "update",
        "RoomUser",
        { roomId, userId, lastSeenAt: roomUser.lastSeenAt },
        "RoomsService",
      );
      this.logger.debug(
        `Updated last seen time for user ${userId} in room ${roomId}`,
        "RoomsService",
      );
    } else {
      this.logger.debug(
        `User ${userId} not found in room ${roomId}, cannot update last seen time`,
        "RoomsService",
      );
    }
  }
   ...
   * @private
   */
  private async formatRoomResponse(
    rooms: Room[],
    userId: number,
  ): Promise<RoomResponseDto[]> {
    this.logger.debug(
      `Formatting response for ${rooms.length} rooms`,
      "RoomsService",
    );
    ...
    const otherUsersByRoomId: Record<number, any> =
      await this.getDirectMessageUser(rooms, userId);
  ...
  private async getDirectMessageUser(rooms: Room[], userId: number) {
    this.logger.debug(
      `Getting other users for ${rooms.length} direct messages`,
      "RoomsService",
    );
    ...
    this.logger.debug(
      `Getting other users for ${directRoomIds.length} direct messages`,
      "RoomsService",
    );
    ...
    if (directRoomIds.length > 0) {
      ...
      this.logger.logDatabase(
        "query",
        "RoomUser",
        { operation: "getDirectMessageUsers", directRoomIds },
        "RoomsService",
      );
  ...
  private async calculateUnreadCounts(
    ...
    this.logger.debug(
      `Calculating unread counts for user ${userId} in ${roomIds.length} rooms`,
      "RoomsService",
    );
  ...
  async updateRoom(
    ...
    if (!room) {
      this.logger.warn(
        `Room with ID ${roomId} not found during update attempt`,
        "RoomsService",
      );
  ...
  async deleteRoom(roomId: number): Promise<boolean> {
    ...
    if (!room) {
      this.logger.warn(
        `Room with ID ${roomId} not found during delete attempt`,
        "RoomsService",
      );
      throw new NotFoundException(`Room with ID ${roomId} not found`);
    }
  ...
  async updateUserRole(
    ...
    userId: number,
    role: RoomRole,
  ): Promise<RoomUserResponseDto> {
    this.logger.debug(
      `Updating role for user ${userId} in room ${roomId} to ${role}`,
      "RoomsService",
    );
    ...
    if (!roomUser) {
      this.logger.warn(
        `User with ID ${userId} not found in room with ID ${roomId} during role update`,
        "RoomsService",
      );
      throw new NotFoundException(
        `User with ID ${userId} not found in room with ID ${roomId}`,
      );
    }
    ...
    this.logger.log(
      `Updated role for user ${userId} in room ${roomId} to ${role}`,
      "RoomsService",
    );
  ...
  async getUserRoomsWithFilters(
    ...
    this.logger.debug(
      `Fetching rooms for user ${userId} with filters: ${JSON.stringify({
        type,
        search,
        page,
        limit,
    ...
    if (search) {
      this.logger.debug(
        `Filtering by search term: "${search}"`,
        "RoomsService",
      );
    ...
    this.logger.debug(
      `Found ${totalItems} rooms total, returning ${paginatedRooms.length} for page ${page}`,
      "RoomsService",
    );
  ...
  async getPublicRooms(
    ...
    this.logger.debug(
      `Fetching public rooms with search: "${
        search || ""
    ...
    if (search) {
      ...
      this.logger.debug(
        `Found ${totalItems} public rooms matching search, returning ${publicRooms.length} for page ${page}`,
        "RoomsService",
      );
    ...
    this.logger.debug(
      `Fetching all public rooms with pagination`,
      "RoomsService",
    );

    const publicRooms = await this.roomRepository.find(baseConditions, {
      orderBy: { createdAt: QueryOrder.DESC },
      limit,
      offset: (page - 1) * limit,
    });
    ...
    this.logger.debug(
      `Found ${totalItems} public rooms total, returning ${publicRooms.length} for page ${page}`,
      "RoomsService",
    );
