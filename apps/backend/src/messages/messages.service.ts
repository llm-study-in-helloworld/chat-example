...
export class MessagesService {
  ...
  async getRoomMessages(roomId: number, limit = 20, offset = 0): Promise<MessageResponseDto[]> {
    ...
    try {
      ...
      this.logger.error(`Error fetching messages for room ${roomId}: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('getRoomMessages', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async getMessage(messageId: number): Promise<MessageResponseDto | null> {
    this.logger.logMethodEntry('getMessage', 'MessagesService');
    const startTime = Date.now();
    
    try {
      this.logger.debug(`Fetching message with ID ${messageId}`, 'MessagesService');
      ...
      if (!message) {
        this.logger.debug(`Message with ID ${messageId} not found`, 'MessagesService');
        return null;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error fetching message with ID ${messageId}: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('getMessage', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async createMessage(data: CreateMessageDto): Promise<MessageResponseDto> {
    this.logger.logMethodEntry('createMessage', 'MessagesService');
    const startTime = Date.now();
    
    try {
      this.logger.debug(`Creating message in room ${data.roomId} by user ${data.senderId}`, 'MessagesService');
      
      // Check if this is a reply to a reply
      if (data.parentId) {
        this.logger.debug(`Checking parent message with ID ${data.parentId}`, 'MessagesService');
        
        const parentMessage = await this.messageRepository.findOne({ id: data.parentId });
        if (!parentMessage) {
          this.logger.warn(`Parent message with ID ${data.parentId} not found`, 'MessagesService');
          throw new NotFoundException('Parent message not found');
        }
        
        // Check if parent message is already a reply
        if (parentMessage.parent) {
          this.logger.warn(`Attempted to reply to a reply (message ID: ${data.parentId})`, 'MessagesService');
      ...
      const message = await this.em.transactional(async (em) => {
        this.logger.logDatabase('transaction', 'Message', { operation: 'create' }, 'MessagesService');
        ...
        this.logger.logDatabase('persist', 'Message', { id: message.id }, 'MessagesService');
        
        // 멘션 처리
        const mentions = this.extractMentions(data.content);
        if (mentions.length > 0) {
          this.logger.debug(`Processing ${mentions.length} mentions in message`, 'MessagesService');
      ...
      this.logger.debug(`Reloading message with ID ${message.id} with all relations`, 'MessagesService');
      ...
      this.logger.log(`Message created: ID ${message.id} in room ${data.roomId} by user ${data.senderId}`, 'MessagesService');
      return dto;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error creating message: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('createMessage', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async updateMessage(messageId: number, userId: number, data: UpdateMessageDto): Promise<MessageResponseDto> {
    this.logger.logMethodEntry('updateMessage', 'MessagesService');
    const startTime = Date.now();
    
    try {
      this.logger.debug(`Updating message ${messageId} by user ${userId}`, 'MessagesService');
      
      const message = await this.messageRepository.findOne({ id: messageId }, {
        populate: ['sender','reactions', 'reactions.user', 'mentions', 'mentions.mentionedUser']
      });
      if (!message) {
        this.logger.warn(`Message with ID ${messageId} not found during update attempt`, 'MessagesService');
        throw new NotFoundException('메시지를 찾을 수 없습니다');
      }
      
      if (message.sender.id !== userId) {
        this.logger.warn(`Unauthorized message update attempt: user ${userId} tried to update message ${messageId} owned by user ${message.sender.id}`, 'MessagesService');
        throw new ForbiddenException('메시지를 수정할 권한이 없습니다');
      }
      
      // 이미 삭제된 메시지인지 확인
      if (message.isDeleted) {
        this.logger.warn(`Attempted to update deleted message with ID ${messageId}`, 'MessagesService');
      ...
      this.logger.logDatabase('update', 'Message', { id: messageId }, 'MessagesService');
      
      // 기존 멘션 제거 및 새 멘션 추가
      await this.em.nativeDelete(Mention, { messageId: messageId });
      this.logger.logDatabase('delete', 'Mention', { messageId }, 'MessagesService');
      
      const mentions = this.extractMentions(data.content);
      if (mentions.length > 0) {
        this.logger.debug(`Processing ${mentions.length} mentions in updated message`, 'MessagesService');
      ...
      this.logger.log(`Message updated: ID ${messageId} by user ${userId}`, 'MessagesService');
      return dto;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error updating message ${messageId}: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('updateMessage', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async deleteMessage(messageId: number, userId: number): Promise<boolean> {
    this.logger.logMethodEntry('deleteMessage', 'MessagesService');
    const startTime = Date.now();
    
    try {
      this.logger.debug(`Deleting message ${messageId} by user ${userId}`, 'MessagesService');
      
      const message = await this.messageRepository.findOne({ id: messageId }, {
        populate: ['sender']
      });
      if (!message) {
        this.logger.warn(`Message with ID ${messageId} not found during delete attempt`, 'MessagesService');
        throw new NotFoundException('메시지를 찾을 수 없습니다');
      }
      
      if (message.sender.id !== userId) {
        this.logger.warn(`Unauthorized message delete attempt: user ${userId} tried to delete message ${messageId} owned by user ${message.sender.id}`, 'MessagesService');
      ...
      await this.em.persistAndFlush(message);
      this.logger.logDatabase('softDelete', 'Message', { id: messageId }, 'MessagesService');
      
      this.logger.log(`Message soft-deleted: ID ${messageId} by user ${userId}`, 'MessagesService');
      return true;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error deleting message ${messageId}: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('deleteMessage', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async toggleReaction(messageId: number, userId: number, emoji: string): Promise<MessageReactionResponseDto | null> {
    this.logger.logMethodEntry('toggleReaction', 'MessagesService');
    const startTime = Date.now();
    
    try {
      this.logger.debug(`Toggling reaction for message ${messageId}, user ${userId}, emoji "${emoji}"`, 'MessagesService');
      
      return this.em.transactional(async (em) => {
        this.logger.logDatabase('transaction', 'MessageReaction', { operation: 'toggle' }, 'MessagesService');
        ...
        // 있으면 제거, 없으면 추가
        if (existingReaction) {
          this.logger.debug(`Removing existing reaction ${existingReaction.id}`, 'MessagesService');
          ...
          this.logger.logDatabase('persist', 'MessageReaction', { id: reaction.id }, 'MessagesService');
          
          this.logger.debug(`Added new reaction to message ${messageId}`, 'MessagesService');
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error toggling reaction for message ${messageId}: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('toggleReaction', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async saveMentions(messageId: number, usernames: string[]): Promise<void> {
    this.logger.logMethodEntry('saveMentions', 'MessagesService');
    const startTime = Date.now();
    ...
    try {
      ...
      for (const username of usernames) {
        ...
        if (user) {
          ...
          this.logger.logDatabase('persist', 'Mention', { messageId, userId: user.id }, 'MessagesService');
        } else {
          this.logger.debug(`Mention for username "${username}" skipped - user not found`, 'MessagesService');
        }
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error saving mentions for message ${messageId}: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    } finally {
      this.logger.logMethodExit('saveMentions', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async canEditMessage(userId: number, messageId: number): Promise<boolean> {
    this.logger.logMethodEntry('canEditMessage', 'MessagesService');
    const startTime = Date.now();
    ...
    try {
      ...
      if (!message) {
        this.logger.debug(`Cannot edit message ${messageId}: message not found`, 'MessagesService');
        return false;
      }
      
      const canEdit = message.sender.id === userId && !message.isDeleted;
      this.logger.debug(`User ${userId} ${canEdit ? 'can' : 'cannot'} edit message ${messageId}`, 'MessagesService');
      return canEdit;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error checking edit permission: ${errorMessage}`, errorStack, 'MessagesService');
      return false;
    } finally {
      this.logger.logMethodExit('canEditMessage', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  async canDeleteMessage(userId: number, messageId: number): Promise<boolean> {
    this.logger.logMethodEntry('canDeleteMessage', 'MessagesService');
    const startTime = Date.now();
    ...
    try {
      ...
      if (!message) {
        this.logger.debug(`Cannot delete message ${messageId}: message not found`, 'MessagesService');
        return false;
      }
      
      const canDelete = message.sender.id === userId && !message.isDeleted;
      this.logger.debug(`User ${userId} ${canDelete ? 'can' : 'cannot'} delete message ${messageId}`, 'MessagesService');
      return canDelete;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error checking delete permission: ${errorMessage}`, errorStack, 'MessagesService');
      return false;
    } finally {
      this.logger.logMethodExit('canDeleteMessage', Date.now() - startTime, 'MessagesService');
    }
  }
  ...
  private async countRepliesForMessages(messageIds: number[]): Promise<Record<number, number>> {
    ...
    try {
      this.logger.debug(`Counting replies for ${messageIds.length} messages`, 'MessagesService');
      ...
      this.logger.logDatabase('query', 'Message', { operation: 'countReplies', messageIds }, 'MessagesService');
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error counting replies: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    }
  }
  ...
  private async formatMessagesResponse(messages: Message[]): Promise<MessageResponseDto[]> {
    ...
    try {
      this.logger.debug(`Formatting response for ${messages.length} messages`, 'MessagesService');
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error formatting message response: ${errorMessage}`, errorStack, 'MessagesService');
      throw error;
    }
  }
