...
export class LogInterceptor implements NestInterceptor {
   ...
   */
  private logRequest(context: ExecutionContext, metrics: PerformanceMetrics): string {
    ...
    if (contextType === 'http') {
      ...
      if (request.user) {
        ...
        this.logger.debug(`Authenticated request from user ${request.user.id}`, metrics.className);
      }
      
      // Log request details with appropriate body redaction for sensitive routes
      const shouldRedactBody = originalUrl.includes('/auth/') 
      ...
      const dataInfo = data ? `event with data of type ${typeof data}` : 'no data';
      
      this.logger.debug(`WebSocket message: ${metrics.methodName} from ${clientInfo} with ${dataInfo}`, metrics.className);
    }
      ...
      metrics.method = 'RPC';
      metrics.path = metrics.methodName;
      this.logger.debug(`RPC operation: ${metrics.methodName}`, metrics.className);
    }
    else {
      requestInfo = `${contextType}:${metrics.methodName}`;
      metrics.method = contextType;
      metrics.path = metrics.methodName;
      this.logger.debug(`Request: ${contextType} ${metrics.methodName}`, metrics.className);
    }
   ...
   */
  private logResponse(context: ExecutionContext, data: any, metrics: PerformanceMetrics): void {
   ...
   */
  private logError(context: ExecutionContext, error: unknown, requestInfo: string, metrics: PerformanceMetrics): void {
   ...
   */
  private logCompletion(requestInfo: string, metrics: PerformanceMetrics): void {
    ...
    this.logger.debug(`Completed ${requestInfo} in ${duration}ms`, metrics.className);
  }
  ...
  private getResponseSummary(data: any): string {
    ...
    try {
      if (typeof data === 'object') {
        ...
        if (Array.isArray(data)) {
          ...
          for (let i = 0; i < sampleSize; i++) {
            const item = data[i];
            types.add(Array.isArray(item) ? 'array' : 
                     (item === null ? 'null' : 
                     (typeof item === 'object' ? (item.constructor?.name || 'object') : typeof item)));
          }
        ...
        try {
          ...
          const typeName = data.constructor && data.constructor !== Object 
          ...
          if (keys.length <= 5) {
            ...
            return `${typeName} with ${keys.length} properties, including: [${sampleKeys.join(', ')}...]`;
          }
        } catch (e) {
      ...
      return `unprocessable response data (${e instanceof Error ? e.message : 'unknown error'})`;
    }
  }
