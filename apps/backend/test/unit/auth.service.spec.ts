...
describe("AuthService", () => {
  ...
  describe("validateUser", () => {
    ...
    it("should throw UnauthorizedException when email is invalid", async () => {
      // Act & Assert
      await expect(
        authService.validateUser("wrong@example.com", testPassword),
  ...
  describe("refreshTokens", () => {
    ...
    it("should throw UnauthorizedException when user is not found", async () => {
      // Act & Assert
      await expect(
        authService.refreshTokens(999, "valid-refresh-token"),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe("logout", () => {
    it("should revoke all refresh tokens and blacklist user tokens", async () => {
      ...
      jest
        .spyOn(refreshTokenService, "revokeAllUserRefreshTokens")
        .mockResolvedValue();
      jest
        .spyOn(tokenBlacklistService, "blacklistUserTokens")
        .mockReturnValue();
      ...
      expect(tokenBlacklistService.blacklistUserTokens).toHaveBeenCalledWith(
        user.id,
      );
    });
