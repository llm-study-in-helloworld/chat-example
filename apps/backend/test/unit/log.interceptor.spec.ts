...
describe("LogInterceptor", () => {
  ...
  it("should handle WebSocket context correctly", () => {
    const wsContext = {
      ...
      switchToWs: jest.fn().mockReturnValue({
        ...
        getData: jest
          .fn()
          .mockReturnValue({ type: "message", content: "test" }),
      }),
    } as unknown as ExecutionContext;
  ...
  it("should properly format response data summary", () => {
    ...
    expect(method([1, "test", { a: 1 }])).toBe(
      "array with 3 items of type(s): number, string, Object",
    );

    // Test objects
    expect(method({})).toBe("empty object");
    expect(method({ id: 1, name: "test" })).toBe(
    ...
    expect(method(largeObj)).toContain(
      "object with 6 properties, including: [a, b, c...",
    );
