...
describe("ChatGateway", () => {
  ...
  describe("handleConnection", () => {
    it("should authenticate user and join their rooms", async () => {
      ...
      jest
        .spyOn(roomsService, "getUserRooms")
        .mockResolvedValue([testRoom1, testRoom2]);
  ...
  describe("handleJoinRoom", () => {
    it("should allow user to join a room they have access to", async () => {
      ...
      // Assert
      expect(roomsService.canUserJoinRoom).toHaveBeenCalledWith({
        userId: testUser1.id,
        roomId: testRoom1.id,
      });
      expect(mockSocket.join).toHaveBeenCalledWith(`room:${testRoom1.id}`);
      expect(roomsService.updateLastSeen).toHaveBeenCalledWith(
        testUser1.id,
        testRoom1.id,
      );
  ...
  describe("handleNewMessage", () => {
    it("should create and broadcast a new message", async () => {
      ...
      jest
        ...
        .mockResolvedValue(testMessageResponse as MessageResponseDto);
      ...
      // Act
      const result = await gateway.handleNewMessage(
    ...
    it("should handle message with mentions and notify mentioned users", async () => {
      ...
      jest
        .spyOn(messagesService, "createMessage")
        .mockResolvedValue(messageWithMentions as MessageResponseDto);
  ...
  describe("handleEditMessage", () => {
    it("should update and broadcast an edited message", async () => {
      ...
      jest
        ...
        .mockResolvedValue(updatedMessage as MessageResponseDto);
      ...
      // Assert
      expect(messagesService.canEditMessage).toHaveBeenCalledWith(
        testUser1.id,
  ...
  describe("handleReaction", () => {
    it("should add a reaction and broadcast it", async () => {
      ...
      mockSocket.data = { user: testUser1 };
      jest
        .spyOn(messagesService, "toggleReaction")
        .mockResolvedValue(
          testReaction as unknown as MessageReactionResponseDto,
        );
      ...
      jest
        .spyOn(messagesService, "getMessage")
        .mockResolvedValue(messageWithReactionDto as MessageResponseDto);
    ...
    it("should handle removing a reaction", async () => {
      ...
      jest
        .spyOn(messagesService, "getMessage")
        .mockResolvedValue(messageWithoutReactions as MessageResponseDto);
    ...
    it("should handle service errors gracefully", async () => {
      ...
      jest
        .spyOn(messagesService, "toggleReaction")
        .mockRejectedValue(new Error("Reaction failed"));
  ...
  describe("handleReplyMessage", () => {
    it("should create and broadcast a reply to a message", async () => {
      ...
      jest
        .spyOn(messagesService, "getMessage")
        .mockResolvedValue(parentMessage);
    ...
    it("should not send notification when replying to own message", async () => {
      ...
      const toUserCalls = jest
        .mocked(mockServer.to!)
        .mock.calls.filter((call) => call[0] === `user:${testUser1.id}`);
