...
describe("RoomsController (e2e)", () => {
  ...
  describe("Feature: Room Management", () => {
    ...
    it("Scenario: User gets list of their rooms", async () => {
      ...
      const groupRoom = response.body.find(
        (room: any) => room.id === createdRoomId,
      );
      expect(groupRoom).toBeDefined();
      expect(groupRoom.isDirect).toBe(false);

      const dmRoom = response.body.find(
        (room: any) => room.id === directMessageRoomId,
      );
  ...
  describe("Feature: Room Membership", () => {
    ...
    it("Scenario: User gets list of room members", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const userFound = response.body.some(
          (member: any) => member.user.id === testUsers[i].id,
        );
    ...
    it("Scenario: Room creator has admin role", async () => {
      ...
      const adminUser = response.body.find(
        (member: any) => member.user.id === testUsers[0].id,
      );
    ...
    it("Scenario: Added members have member role by default", async () => {
      ...
      const memberUser = response.body.find(
        (member: any) => member.user.id === testUsers[1].id,
      );
    ...
    it("Scenario: Admin can promote a member to admin", async () => {
      ...
      const promotedUser = response.body.find(
        (member: any) => member.user.id === testUsers[1].id,
      );
  ...
  describe("Feature: Direct Messaging", () => {
    ...
    it("Scenario: DM rooms show correct other user details", async () => {
      ...
      // Find the DM room
      const dmRoom = roomsResponse.body.find(
        (room: any) => room.id === dmRoomId,
      );
      expect(dmRoom).toBeDefined();
      ...
      const dmRoomForUser2 = user2RoomsResponse.body.find(
        (room: any) => room.id === dmRoomId,
      );
