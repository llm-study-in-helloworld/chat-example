...
} from "../../src/gateway/dto/socket-response.dto";
...
describe("ChatGateway (e2e)", () => {
  ...
  describe("Room Management", () => {
    ...
    it("should allow joining a room with access permissions", async () => {
      ...
      const response = await emitAndWait<SocketResponse<SocketSuccessDto>>(
        userSocket,
        "join_room",
        { roomId },
      );
  ...
  describe("Messaging", () => {
    ...
    beforeEach(async () => {
      ...
      for (const clientKey in socketClients) {
        await emitAndWait<SocketResponse<SocketSuccessDto>>(
          socketClients[clientKey],
          "join_room",
          { roomId },
        );
      }
    ...
    it("should send and receive messages in a room", async () => {
      ...
      const messagePromise = waitForEvent<MessageResponseDto>(
        receiverSocket,
        "new_message",
      );
    ...
    it("should handle replies to messages", async () => {
      ...
      const parentMessage = await emitAndWait<MessageResponseDto>(
        senderSocket,
        "new_message",
        message,
      );
      ...
      const newMessagePromise = waitForEvent<MessageResponseDto>(
        originalSenderSocket,
        "new_message",
      );

      // Set up listener for reply alert event
      const replyAlertPromise = waitForEvent<{
        messageId: number;
        parentId: number;
        roomId: number;
      }>(
    ...
    it("should reject replies to non-existent messages", async () => {
      ...
      // Then they should receive an error
      expect(response).toHaveProperty("error");
      expect((response as SocketErrorDto).error).toContain(
        "답장할 메시지를 찾을 수 없습니다",
      );
    });
    ...
    it("should edit messages", async () => {
      ...
      const updatePromise = waitForEvent<MessageResponseDto>(
        otherUserSocket,
        "message_updated",
      );
  ...
  describe("Reactions", () => {
    ...
    it("should add reactions to messages", async () => {
      ...
      const reactionPromise = waitForEvent<ReactionUpdateEventDto>(
        socketClients["user1"],
        "reaction_updated",
      );
      ...
      const response = await emitAndWait<ReactionResponseDto>(
        socketClients["user2"],
        "react_message",
        reactionData,
      );
    ...
    it("should toggle reactions when the same user reacts twice", async () => {
      ...
      await emitAndWait<ReactionResponseDto>(
        reactorSocket,
        "react_message",
        reactionData,
      );
      ...
      const thumbsUpReaction = reactionEvent.reactions.find(
        (r) => r.emoji === "👍" && r.userId === testUsers[1].id,
      );
  ...
  describe("User Presence", () => {
    it("should notify users when someone goes offline", async () => {
      ...
      const presencePromise = waitForEvent<UserPresenceEventDto>(
        socketClients["user1"],
        "user_presence",
      );
